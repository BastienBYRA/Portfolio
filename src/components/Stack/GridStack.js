...
class GridStack extends React.Component {
  constructor(props) {
    ...
    this.firstDiv_GridColSpan =
      "flex p-6 flex-col backdrop-blur-md bg-black/20 hover:bg-black/40 duration-300 border-transparent border-2 hover:border-slate-600 rounded-lg relative";
    this.secondDiv_GridNbElement =
      "grid grid-flow-row gap-y-4 gap-x-1 items-center";
    this.state = {
      langagesLength: null,
          ...
          <div className={this.state.grid_col_span}>
            <div className={this.state.grid_nb_elements}>
              {this.langages.map((langage) => {
                return <Langage2 name={langage.name} img={langage.img} />;
              })}
            </div>
